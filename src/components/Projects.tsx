			...
			<div className="container px-6 max-w-6xl mx-auto">
				...
				<div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
					{projects.map((project, index) => (
						...
						>
							<Card
								className="group hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 bg-card/50 backdrop-blur border-border/50 rounded-xl overflow-hidden cursor-pointer h-full flex flex-col" // Added h-full and flex layout
							...
							>
								...
								<CardHeader className="pb-3">
									<CardTitle className="text-xl group-hover:text-chart-1 transition-colors h-[28px] line-clamp-1">
										{project.title}
									</CardTitle>
									<CardDescription className="text-muted-foreground leading-relaxed h-[96px] overflow-hidden line-clamp-4">
										{project.description}
									</CardDescription>
								</CardHeader>
								...
								<CardFooter className="pt-0 gap-3 mt-auto">
									...
									<Button
										size="sm"
										variant="outline"
										className="border-chart-1/30 hover:border-chart-1 hover:bg-chart-1/10 text-foreground"
										asChild
									>
										<a
											href={project.githubUrl}
